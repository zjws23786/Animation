...
public class GifOpenHelper {
    ...
    // to store *.gif data, Bitmap & delay 存储GIF数据，对象包括位图和延迟
    class GifFrame {
    ...
    // to define some error type 定义相关标准
    ...
    protected int frameCount;  //gif帧数
    ...
    protected void readContents() {
        ...
        while (!(done || err())) {
            int code = read();
            switch (code) {
                case 0x2C: // image separator  图像分离器
                    readImage();
                    break;
                case 0x21: // extension  延伸
                    code = read();
                    switch (code) {
