...
public class AnimShopButton extends View {
    ...
    //加减的圆的Path的Region，主要的作用是提供了点击事件
    ...
    protected void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mAddRegion = new Region();  //定义一个区域对象
        mDelRegion = new Region();
        mAddPath = new Path();     //路径
        mDelPath = new Path();

        mAddPaint = new Paint(Paint.ANTI_ALIAS_FLAG);  //抗锯齿
        if (isAddFillMode) {
        ...
        mAniDel.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                if (mCount == 0) {
                    Log.d("hjz", "现在还是0onAnimationEnd() called with: animation = [" + animation + "]");
                    if (mAnimExpandHint != null && !mAnimExpandHint.isRunning()) {
    ...
    private void initDefaultValue(Context context) {
        //加减之间的距离
        mGapBetweenCircle = TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, 34,
                context.getResources().getDisplayMetrics());

        //+默认是填充色
        isAddFillMode = true;
        mAddEnableBgColor = 0xFFFFDC5B;   //正常情况背景
        mAddEnableFgColor = Color.BLACK;  //正常情况前景
        mAddDisableBgColor = 0xff979797;  //不可用背景
        mAddDisableFgColor = Color.BLACK; //不可用前景

        //-默认是描边
        isDelFillMode = false;
        mDelEnableBgColor = 0xff979797;  //正常情况背景
        mDelEnableFgColor = 0xff979797;  //正常情况前景
        mDelDisableBgColor = 0xff979797; //不可用背景
        mDelDisableFgColor = 0xff979797; //不可用前景
    ...
    protected void onDraw(Canvas canvas) {
        if (isReplenish) {
            ...
            canvas.drawText(mReplenishText, baseX, baseY, mReplenishPaint);//文字绘制是从左下角开始
            return;
        }
    ...
    protected void onAddClick() {
        if (mCount < mMaxCount) {
            ...
            if (null != mOnAddDelListener) {
                ...
                mOnAddDelListener.onAddSuccess(mCount,rawX,rawY);
            }
        } else {
            if (null != mOnAddDelListener) {
